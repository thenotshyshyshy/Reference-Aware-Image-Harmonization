...
def load_pretrained_model(model, opt):
    # Check if the 'latest_net_G.pth' file exists
    latest_model_path = os.path.join(opt.checkpoints_dir, opt.name, 'latest_net_G.pth')
    if os.path.exists(latest_model_path):
        print(f'Loading model from {latest_model_path}')
        model.load_networks('latest')  # Use 'latest' suffix to load latest model
        print(f'Model loaded from {latest_model_path}')
    else:
        print('No pre-trained model found or continue training flag is not set.')

def save_metrics_to_file(path, metrics):
    os.makedirs(os.path.dirname(path), exist_ok=True)
    with open(path, 'a') as f:  # 以追加模式打开文件
        f.write(f"Epoch: {metrics['epoch']}, MSE: {metrics['MSE']}, PSNR: {metrics['PSNR']}\n")
    ...
    for i, data in tqdm(enumerate(test_dataset), total=len(train_dataloader)):
        ...
        for i_img in range(real.size(0)):
            ...
            if epoch >= 100:
                ...
                save_dir = os.path.join(opt.checkpoints_dir, 'evaluate', str(epoch_number), 'results', basename)
...
if __name__ == '__main__':
    ...
    max_psnr = 0
    max_epoch = 1
    metrics_folder = os.path.join(opt.checkpoints_dir, opt.name, 'metrics')
    ...
    for epoch in tqdm(range(opt.load_iter+1, opt.niter + opt.niter_decay + 1)):
        ...
        save_metrics_to_file(metrics_file, {'epoch': epoch, 'MSE': epoch_mse, 'PSNR': epoch_psnr})
