...
def get_act_dconv(act, dims_in, dims_out, kernel, stride, padding, bias):
    ...
    return nn.Sequential(*conv)
def get_act_odconv(act, odconv_layer):
    return nn.Sequential(
        act,          # 先应用激活函数
        odconv_layer  # 再应用 ODConv 卷积层
    )
leaky_relu = nn.LeakyReLU(0.2, True)
def odconv4x4(in_planes, out_planes, stride=2, reduction=0.0625, kernel_num=1):
    return ODConv2d(in_planes, out_planes, kernel_size=4, stride=stride, padding=1,
...
def odconv_transpose4x4(in_planes, out_planes, stride=2, output_padding=1, reduction=0.0625, kernel_num=1):
    return ODConvTranspose2d(in_planes, out_planes, kernel_size=4, stride=stride, padding=1, output_padding=output_padding,
                             reduction=reduction, kernel_num=kernel_num)

def odconv_transpose3x3(in_planes, out_planes, stride=1, output_padding=0, reduction=0.0625, kernel_num=1):
    return ODConvTranspose2d(in_planes, out_planes, kernel_size=3, stride=stride, padding=1, output_padding=output_padding,
                             reduction=reduction, kernel_num=kernel_num)
...
class HDNet(nn.Module):
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=RAIN, 
        ...
        self.model_layer1 = get_act_odconv(leaky_relu, odconv4x4(ngf, ngf*2, stride=2, reduction=0.0625, kernel_num=1))
        ...
        self.model_layer2 = get_act_odconv(leaky_relu, odconv3x3(ngf*2, ngf*4, stride=1, reduction=0.0625, kernel_num=1))
        ...
        self.model_layer3 = get_act_odconv(leaky_relu, odconv4x4(ngf*4, ngf*8, stride=2, reduction=0.0625, kernel_num=1))
        ...
        self.model_layer4 = get_act_odconv(leaky_relu, odconv3x3(ngf*8, ngf*8, stride=1, reduction=0.0625, kernel_num=1))
        ...
        self.model_layer5 = get_act_odconv(leaky_relu, odconv4x4(ngf*8, ngf*8, stride=2, reduction=0.0625, kernel_num=1))
        ...
        self.model_layer6 = get_act_odconv(leaky_relu, odconv3x3(ngf*8, ngf*8, stride=1, reduction=0.0625, kernel_num=1))
        ...
        self.model_layer71 = get_act_odconv(leaky_relu, odconv3x3(ngf*8, ngf*8, stride=1, reduction=0.0625, kernel_num=1))
        ...
        #self.model_layer11        = odconv4x4(ngf*16, ngf*4, stride=2, reduction=0.0625, kernel_num=1)
        self.model_layer11norm    = norm_type_list[norm_type_indicator[11]](ngf*4)

        if use_attention:
            self.model_layer11att = DRConv2d(ngf*8, ngf*8, 1, 3)
            
        self.model_layer12        = get_act_dconv(nn.ReLU(True), ngf*8, ngf*2, 3, 1, 1, False)
        #self.model_layer12        = odconv3x3(ngf*8, ngf*2, stride=1, reduction=0.0625, kernel_num=1)
        self.model_layer12norm    = norm_type_list[norm_type_indicator[12]](ngf*2)

        if use_attention:
            self.model_layer12att = DRConv2d(ngf*4, ngf*4, 1, 3)
            
        self.model_layer13        = get_act_dconv(nn.ReLU(True), ngf*4, ngf, 4, 2, 1, False)
        #self.model_layer13        = odconv4x4(ngf*4, ngf, stride=2, reduction=0.0625, kernel_num=1)
        self.model_layer13norm    = norm_type_list[norm_type_indicator[13]](ngf)
        if use_attention:
            self.model_layer13att = DRConv2d(ngf*2, ngf*2, 1, 3)
        ...
        self.cdim1 = CDIM(ngf*8)
        self.norm1 = nn.LayerNorm(ngf*8)
        '''self.rg_sa = RG_SA(
            dim=ngf*8,
            num_heads=8,
            qkv_bias=True,
        ...
        '''self.attn = RG_SA(
                dim=ngf*8, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0,
                proj_drop=0, c_ratio=0.5
            )'''
        '''self.attn = L_SA(
                dim=ngf*8, split_size=[2,4], shift_size=[1,2], num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0,
                drop=0, idx=0, reso=64, rs_id=0
            )'''
        #self.attn = DRConv2d(ngf*8, ngf*8, 1, 3)
        
    def forward(self, x, mask,cdp):
        ...
        B, C, H, W = x71.size()
        x71_flat = x71.view(B, C, -1).permute(0, 2, 1)  #  (B, H * W, C)
        x71_normalized = self.norm1(x71_flat)  #  (B, H * W, 32)
        x71_fused = self.cdim1(x71_normalized, cdp)  # (B, H * W, 32)
        #x71 = self.LD(x71, mask,x71_fused.permute(0, 2, 1).view(B, C, H, W))
        x71=x71+x71_fused.permute(0, 2, 1).view(B, C, H, W)
        
        #x71=self.attn(x71+x71_fused.permute(0, 2, 1).view(B, C, H, W),mask)
        #x71 = x71 + self.attn(x71_fused, H, W).permute(0, 2, 1).view(B, C, H, W)
        ...
        #B, C, H, W = ox2.size()
        #ox2_flat = ox2.view(B, C, -1).permute(0, 2, 1)  #  (B, H * W, C)
        #ox2_normalized = self.norm1(ox2_flat)  #  (B, H * W, 32)
        #ox2_fused = self.cdim1(ox2_normalized, cdp)  # (B, H * W, 32)
        #ox2=ox2+ox2_fused.permute(0, 2, 1).view(B, C, H, W)
